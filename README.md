...
- **웹사이트 빌드 방식 선택** (신규)
  - **Astro Starlight** — 세련된 최신 디자인, 반응형·다크모드 내장. 첫 빌드 3~6분, 이후 캐시 재활용
  - **MkDocs Material** — 빠른 빌드(10~30초), 클래식 문서 스타일. 메모리 부담 적음
...
|------|------|-----------|---------------|
| Node.js | Starlight 빌드 (기본) | 이미 설치됨 | 이미 설치됨 |
| [MkDocs](https://www.mkdocs.org) | 웹사이트 빌드 (대체) | `pip install mkdocs mkdocs-material` | 동일 |
| [Pandoc](https://pandoc.org) | Word 변환 | `brew install pandoc` | [다운로드](https://pandoc.org/installing.html) |
| [GitHub CLI](https://cli.github.com) | GitHub 배포 (권장 인증) | `brew install gh` → `gh auth login` | [다운로드](https://cli.github.com) → `gh auth login` |

> **GitHub Pages 배포**는 `gh auth login`을 한 번 실행한 뒤 Step 5에서 바로 사용할 수 있습니다. `gh` 설치가 번거로우면 `.env`에 `GITHUB_TOKEN=ghp_...`을 넣어도 됩니다. 자세한 방법은 [INSTALL.md의 GitHub Pages 배포](INSTALL.md#github-pages로-배포하기-로컬) 참고.

---

## 포트폴리오 자동 관리 (선택)

포트폴리오 관리는 **공유 포트폴리오 리포지토리**가 있을 때만 의미 있는 기능입니다. `.env`에 `PORTFOLIO_GITHUB_TOKEN`을 설정한 환경(주로 웹 배포판)에서만 자동 반영되고, 로컬에서는 GitHub Pages 배포만 수행되고 포트폴리오 단계는 조용히 스킵됩니다(배포 자체에는 영향 없음).

본인만의 포트폴리오 리포지토리를 운영하시려면 [예시 포트폴리오](https://greatsong.github.io/eduflow-portfolio/) 구조를 참고해 별도로 세팅하시면 됩니다.
